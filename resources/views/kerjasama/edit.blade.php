        ...
        <div class="card-body">
            <form action="{{ route('kerjasamas.update', ['kerjasama' => $kerjasama->id]) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" value="{{ $kerjasama->nama_kerja_sama }}" name='nama_kerja_sama'
                ...
                <div class="row">
                    <div class="form-group col-lg-6">
                        ...
                        <input type="date" name="tanggal_mulai" value="{{ $kerjasama->tanggal_mulai }}" id=""
                            class="form-control">
                        @error('tanggal_mulai')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group col-lg-6">
                        ...
                        <input type="date" name="tanggal_sampai" value="{{ $kerjasama->tanggal_sampai }}" id=""
                            class="form-control">
                    ...
                    <select class="form-control select2" name='nama_mitra'>
                        @foreach ($mitras as $data)
                            <option value="{{ $data->id }}"
                                {{ $data->id == $kerjasama->mitra_id ? 'selected' : '' }}>{{ $data->id }} -
                    ...
                    <div class="form-group col-lg-6">
                        ...
                        <select class="form-control" name='nama_kategori'>
                            @foreach ($kategoris as $data)
                                <option value="{{ $data->id }}"
                                    {{ $data->id == $kerjasama->kategori_id ? 'selected' : '' }}>{{ $data->id }} -
